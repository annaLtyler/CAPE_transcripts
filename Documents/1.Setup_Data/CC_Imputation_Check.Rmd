...
if(!data.loaded){
    ...
    bw <- as.matrix(read.csv(file.path(cc.dir, "Weight.Summarized.csv"), header = TRUE, row.names = 1))
    chem.mat <- read.csv(file.path(cc.dir, "Blood.Chemistries.Summarized.csv"), row.names = 1)
    ...
    do.pheno <- read.csv(here("Data", "DO_clinical_phenotypes.csv"))
...
```{r do_trait_cor}
do.imputed <- do.imputed.local
do.imputed <- do.imputed.genetic
...
boxplot(abs(do.cor.mat), las = 2)

max.cor <- apply(do.cor.mat, 1, max)
test <- names(local.genetic.cor)[intersect(which(local.genetic.cor > 0.4), which(max.cor > 0.4))]
test <- names(local.genetic.cor)[which(max.cor > 0.5)]
plot(local.genetic.cor, max.cor)

par(mfrow = c(2,2))
plot.with.model(do.imputed.local[,test[1]], do.imputed.genetic[,test[1]], report = "cor.test",
    xlab = "Local model", ylab = "Genetic model")
plot.with.model(do.imputed.local[,test[1]], adj.do.pheno[rownames(do.imputed.local),"weight_6wk"],
    xlab = "Local Model", ylab = "Weight", report = "cor.test")
plot.with.model(do.imputed.genetic[,test[1]], adj.do.pheno[rownames(do.imputed.local),"weight_6wk"],
    xlab = "Genetic Model", ylab = "Weight", report = "cor.test")
plot.with.model(do.imputed.local[,test[1]], adj.do.pheno[rownames(do.imputed.local),"weight_6wk"],
    xlab = "Genetic Model", ylab = "Weight", report = "cor.test")
...
```{r do_cc_comparison}
do.cor.mat <- do.local.cor.mat
do.cor.mat <- do.genetic.cor.mat
...
pheatmap(comparison.cor, display_numbers = TRUE)

hist(comparison.cor);abline(v = 0, col = "red", lwd = 3)

plot.with.model(do.cor.mat[common.transcripts,"weight_6wk"], 
    cc.cor.mat[common.transcripts,"weight.cor"], xlab = "DO Weight 6 wk",
    ylab = "CC Final Weight", report = "cor.test")
```