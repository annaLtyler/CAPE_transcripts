...
Read in measured CC-RIX gene expression. Also read in DO data.
We will use that to look at imputation based on LOD score in the DO.
...
#adj.do.expr <- do.adj.expr <- adjust(do.expr$data$norm, do.expr$covar.matrix)
```
...
Group the true expression based on the CC-RIX crosses.
...
get_coef_by_allele <- function(transcript.chr, transcript.pos, mouse.expr, 
    ...
    if(plot.results){
        par(mfrow = c(1,2))
        plot(rankZ(allele.coef[,2]), rankZ(mouse.coef), col = CCcolors, pch = 16, cex = 2,
            main = "Comparison of allele fit and fit1")
        abline(0,1)
        coef.order <- order(mouse.coef)
...
#rnd.transcript <- sample(common.transcripts, 1)
plot_do_cc_coef <- function(transcript.id, bp.buffer = 4e6, 
    ...
    cc.kinship = FALSE, plot.results = TRUE){
        ...
        test_do_coef <- get_coef_by_allele(transcript.chr, transcript.pos, 
...
if(is.interactive){
    ...
    plot_do_cc_coef(rnd.transcript, plot.results = TRUE, 
        cor.type = "Spearman", do.kinship = FALSE, cc.kinship = FALSE)
    plot_do_cc_coef(rnd.transcript, plot.results = TRUE, 
        cor.type = "Spearman", do.kinship = FALSE, cc.kinship = TRUE)
    plot_do_cc_coef(rnd.transcript, plot.results = TRUE, 
