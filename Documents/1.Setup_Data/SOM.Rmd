...
for(k in 1:length(u_k)){
  ...
  k.gene.mat[,k] <- k.gene
  k.cor <- apply(adj.pheno[pheno.locale,], 2, function(x) cor(x, k.gene[metagene.locale], 
...
for(i in 1:length(metagene.peak.lod.scans)){
  ...
  if(peak.chr != "X"){
    ...
    plot_coefCC(metagene.peak.allele.scans[[i]], map = map);abline(v = peak.pos)
    cat("\n\n")
  }
}
```
