...
```{r meta_eig}
eig.kmeans.file <- here("Results", "SOM", paste0("Cluster_Eigengenes_", tissue.name, ".RDS"))

if(!file.exists(eig.kmeans.file)){
  ...
  for(i in 1:length(kmeta)){
    ...
    #library(umap)
    #Utest <- umap(cluster.expr)
    
    #library(Rtsne)
    #norm <- normalize_input(cluster.expr)
    #nodup1 <- unique(norm)
    #nodup2 <- unique(t(nodup1))
    #tsn <- Rtsne(nodup2)

    #par(mfrow = c(1,3))
    #plot.decomp(cluster.expr, main = "SVD")
    #plot(Utest$layout, main = "Umap")
    #plot(tsn$Y[,1], tsn$Y[,2], main = "TSNE")
  ...
  saveRDS(eig.kmeans, eig.kmeans.file)
}else{
  eig.kmeans <- readRDS(eig.kmeans.file)
}
```
...
if(!file.exists(pc.scan.file)){
  ...
  for(i in 1:length(eig.kmeans)){
    ...
    #plot(cluster.scans[[i]], lodcol = 1, map = map)
    #plot(cluster.scans[[i]], lodcol = 2, map = map)
  }
...
The following plots examing the correlation between gene clusters
and traits.

```{r trait_cluster_cor}
common.ind <- intersect(rownames(num.pheno), rownames(adj.expr))
expr.idx <- match(common.ind, rownames(adj.expr))
pheno.idx <- match(common.ind, rownames(num.pheno))

#Since I mapped to eigengenes, make a correlation matrix
#for each
trait.cluster.cor <- vector(mode = "list", length = 2)
names(trait.cluster.cor) <- c("PC1", "PC2")
for(i in 1:length(trait.cluster.cor)){
  cor.mat <- matrix(NA, ncol = ncol(num.pheno), nrow = length(kmeta))
  colnames(cor.mat) <- colnames(num.pheno)
  rownames(cor.mat) <- names(kmeta)
  for(k in 1:length(eig.kmeans)){
    cluster.eig <- eig.kmeans[[k]]
    trait.cor <- apply(num.pheno, 2, function(x) cor(x[pheno.idx], cluster.eig[,i], 
    use = "pairwise.complete.obs", method = "spearman"))
    cor.mat[k,] <- trait.cor
  }
  trait.cluster.cor[[i]] <- cor.mat
}

pheatmap(t(trait.cluster.cor[[1]]), main = "Trait Correlations with Cluster PC1")
pheatmap(t(trait.cluster.cor[[2]]), main = "Trait Correlations with Cluster PC2")
```
