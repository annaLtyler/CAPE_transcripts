...
qtl.geno <- genoprobs[[chr]][,,qtl.markers]
```

For each marker in the QTL, check to see how well the up and down
alleles separate animals.

1. We separate each marker into the up and down alleles.
2. We sum across rows to give each individual a total genotype 
  score for the number of up alleles and number of down alleles 
  it has
3. We bin each individual into one of three groups based on whether 
  it has mostly up alleles, mostly down alleles, or an even split of both.
4. Collect the bin assignment for each animal across all markers in the qtl.

The following heatmap shows the bin assigned to each animal across all markers.
The binning clusters the animals into three groups.

```{r sep_geno}

up.alleles <- c("D", "C", "B", "A")
down.alleles <- c("G", "F", "H", "E")

all.up <- lapply(1:dim(qtl.geno)[3], function(x) qtl.geno[,up.alleles,x])
all.down <- lapply(1:dim(qtl.geno)[3], function(x) qtl.geno[,down.alleles,x])
...
for(i in 1:length(qtl.markers)){
  geno.groups <- vector(mode = "list", length = 3)
  names(geno.groups) <- c("up", "mid", "down")
  marker.up <- bin.vector(rowSums(all.up[[i]]))
  marker.down <- bin.vector(rowSums(all.down[[i]]))
  #plot(marker.up, marker.down);abline(v = c(low.thresh, high.thresh), h = c(low.thresh, high.thresh))
  marker.table <- cbind(marker.up, marker.down)
  geno.groups[[1]] <- which(apply(marker.table, 1, function(x) x[1] > x[2]))
  geno.groups[[2]] <- which(apply(marker.table, 1, function(x) x[1] == x[2]))
  geno.groups[[3]] <- which(apply(marker.table, 1, function(x) x[1] < x[2]))
  all.grouped.markers[[i]] <- geno.groups
}

#track the position of each animal over all markers
ind.groups <- lapply(rownames(adj.pheno), 
  function(y) sapply(all.grouped.markers, function(x) grep(y, x)))
group.ind <- sapply(1:3, function(y) sapply(ind.groups, function(x) length(which(x == y))))
colnames(group.ind) <- names(geno.groups)
#barplot(t(group.ind), col = c("red", "blue", "green"))
pheatmap(group.ind)
```

We then use voting to assign each animal to a bin overall. They 
are assigne to the bin in which they were placed for more than
50% of the markers in the qtl. 
...
group.labels <- rep(NA, nrow(adj.pheno))
for(i in 1:length(assigned.groups)){
  group.labels[assigned.groups[[i]]] <- names(geno.groups)[i]
}
#assign leftovers to the mid group
#which(is.na(group.labels))
group.labels[which(is.na(group.labels))] <- "mid"
```
