...
The imputed gene expression (from Matt using the local 
genetic effect in the DO), is our best guess at the 
...
needed.packages <- c("gProfileR", "pheatmap", "qtl2", "RColorBrewer", 
    "biomaRt", "gprofiler2", "stringr", "vctrs")
...
expr.decomp <- plot.decomp(adj.cc.expr)
...
factors. The all look weirdly inflated to me. Do we need to take kinship 
into account here? We will look at this below after creating the kinship
matrix.
...
imputed transcript, and all the interactions between imputed transcript and
...
```{r num_sig}
sig.transcripts <- apply(fdr, 2, function(x) rownames(all.coef)[which(x <= 0.1)])
sig.up <- lapply(1:ncol(fdr), function(x) rownames(all.coef)[intersect(which(fdr[,x] <= 0.1), which(all.coef[,x] > 0))])
sig.down <- lapply(1:ncol(fdr), function(x) rownames(all.coef)[intersect(which(fdr[,x] <= 0.1), which(all.coef[,x] < 0))])
...
```{r up_down_treat, fig.width = 8, fig.height = 7, results = "asis"}
...
## Expression of genes changed by metformin

```{r clustering_differences, eval = FALSE}
met.effect <- rownames(fdr)[which(fdr[,"Treatment"] <= 0.1)]
met.expr <- cc.measured[,met.effect]
adj.met.expr <- adjust(met.expr, dummy_covar(matched.covar[,c("Sex", "Diet")]))
adj.met.expr <- met.expr
scaled.expr <- apply(adj.met.expr, 2, rankZ)

treat.locale <- which(matched.covar[,"Treatment"] == "Metformin")
none.locale <- which(matched.covar[,"Treatment"] == "None")

treat.df <- data.frame(apply(matched.covar[,c("Treatment", "Diet", "Sex")], 2, as.factor))

treatment.idx <- lapply(c("None", "Metformin"), function(x) which(matched.covar[,"Treatment"] == x))
row.order <- lapply(treatment.idx, function(x) hclust(dist(scaled.expr[x,]))$order)
col.order <- hclust(dist(t(scaled.expr)))$order 

full.row.order <- unlist(lapply(1:length(treatment.idx), function(x) treatment.idx[[x]][row.order[[x]]]))

pheatmap(scaled.expr[full.row.order,col.order], annotation_row = treat.df, 
...
```{r model_kin, eval = FALSE}
test.K <- expand_K(cc.K, rownames(adj.cc.expr), manifest)
test.kin <- test.K[[20]]
kin.decomp <- plot.decomp(test.K[[19]])
kin.pc <- kin.decomp$u
colnames(kin.pc) <- c("PC1", "PC2")

matched.covar <- get.xz(adj.cc.expr, mouse.info)$Z

model.coef.file <- here("Results", "CC-RIX_Expression", paste0("model_kin_coef_", tissue.name, ".RDS"))
model.p.file <- here("Results", "CC-RIX_Expression", paste0("model_kin_p_", tissue.name, ".RDS"))

if(!file.exists(model.coef.file) || !file.exists(model.p.file) || delete_previous){
    pc.coef <- pc.p <- matrix(NA, nrow = ncol(adj.cc.expr), ncol = 10)
    rownames(pc.coef) <- rownames(pc.p) <- colnames(adj.cc.expr)

    for(i in 1:ncol(adj.cc.expr)){
        if(is.interactive){report.progress(i, ncol(adj.cc.expr))}

        df <- data.frame(cbind(adj.cc.expr[,i], kin.pc, as.factor(matched.covar[,"Sex"]), 
            as.factor(matched.covar[,"Diet"]), as.factor(matched.covar[,"Treatment"]), 
            as.factor(matched.covar[,"User.Defined.Strain"])))
        colnames(df) <- c("measured_expression", "PC1", "PC2", "Sex", "Diet", "Treatment", "Strain")

        model <- lm(measured_expression~PC1+PC2+Sex+Diet+Treatment+Strain+Sex:Strain+Diet:Strain+Treatment:Strain, data = df)
        #summary(model)

        model.sum <- summary(model)
        pc.coef[i,] <- model.sum$coefficients[,"Estimate"]
        pc.p[i,] <- model.sum$coefficients[,"Pr(>|t|)"]
    }
    colnames(all.coef) <- colnames(all.p) <- rownames(model.sum$coefficients)
    saveRDS(all.coef, model.coef.file)
    saveRDS(all.p, model.p.file)
