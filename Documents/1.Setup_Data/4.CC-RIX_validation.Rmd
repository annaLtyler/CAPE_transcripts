...
#manual.CT.Med = "Med"; tissue.name <- "Adipose"; exp.name = "tissue_sep_full_imp_weight_adjusted" #exp.name tells which Mediation results folder to look in for the transcript loadings
...
if(manual.CT.Med == "Med"){
    tissue.loadings <- readRDS(here("Results", "High_Dim_Med", exp.name, "Loadings_Transcripts.RDS"))
    tissue.idx <- which(names(tissue.loadings) == tissue.name)
    expr.vector <- tissue.loadings[[tissue.idx]][,1]
