...
validation of the Sash3 hypothesis developed in the DO CCA analysis. 
...
#set whether to set genes by hand or to use a CT from the DO study
#manual.or.CT <- "manual"; gene.name = "Aldh2"
#manual.or.CT <- "manual"; gene.name <- c("Ifi44","Trim30a","Usp18","Ifit1","Ifit3","Oasl2","Rtp4","Irf7","Cxcl10","Ube2l6","Rnf213","Bst2","Stat1","Isg15","Lgals3bp","Rnf213","Parp12","Samd9l","Apol9a","Gbp2","Irgm2","Xaf1","Ifi205","Ifi203","Stat2","Ifih1","Oas1g","Apol9b","Sp100","Gbp6","Ddx58","Gbp3","Ifi204","Tor3a","Dtx3l","Oas1b","Ccl5","Parp14","Agrn","Tgtp1","Cmpk2","Nell2","Eif2ak2")

manual.or.CT = "CT"; tissue.name <- "Liver"; CT <- 4
...
if(manual.or.CT == "CT"){
    ...
    if(use.imputed){
        do.expr <- readRDS(here("Data", "imputed", paste0("Adjusted_Expression_", exp.name, ".RDS")))
    }else{
...
treat_effect <- function(contrast = c("Diet", "Treatment")){
    ...
    for(tx in 1:length(chem.matched.expr)){
        ...
        for(g in 1:nrow(no_contrast_groups)){
            ...
            a <- boxplot(tx.expr[group.idx]~matched.chem[[tx]][group.idx,contrast],
            ylab = "Expression", xlab = "", 
            main = paste0(paste(no_contrast_groups[g,], collapse = " "), " (p = ", signif(p, 2), ")"))
            group.means <- colMeans(a[[1]])
            names(group.means) <- a$names
...
```{r diet_effect, fig.height = 8, fig.width = 8, results = "asis"}
...
```{r met_effect, fig.height = 8, fig.width = 8, results = "asis"}
