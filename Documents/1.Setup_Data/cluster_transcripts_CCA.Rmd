...
if(!file.exists(expr.decomp.file)){
    expr.decomp <- plot.decomp(all.expr, plot.results = FALSE, pc = 5)
...
The following plots show the top 2 PCs from all tissue gene expression,
and the top 5 PCs from the trait matrix plotted against each other.
...
trait.decomp <- plot.decomp(cca.pheno, pc = 5)
overall.cor.mat <- matrix(NA, nrow = 5, ncol = 5)
...
for(i in 1:length(tissue.adj.expr)){
    tissue.cor.mat <- matrix(NA, nrow = 5, ncol = 5)
    
    cat(tissue.names[i], "\n")
    tissue.decomp <- plot.decomp(tissue.adj.expr[[i]], plot.results = FALSE, pc = 5)
