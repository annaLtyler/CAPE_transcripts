...
for(i in 1:length(matched.mats)){
    ...
    par(mfrow = c(2,2))
    imageWithText(perm.results$Z, col.scale = "red", main = "Z statistics", col.text.shift = 1)
    imageWithText(perm.results$Cor, col.scale = "blue", main = "Correlation", col.text.shift = 1)
...
sig_loadings <- function(estimate.mat, fdr = 0.05){ 
...
for(i in 1:length(tissue.names)){
    boot.CCA.results[[i]]$u <- get_agg_loadings(tissue.boot.results = tissue.transcript.boots[[i]], 
        tissue.results = tissue.CCA.results[[i]], trait.or.trans = "trans", fdr = 0.01)
    boot.CCA.results[[i]]$v <- get_agg_loadings(tissue.boot.results = tissue.trait.boots[[i]], 
        tissue.results = tissue.CCA.results[[i]], trait.or.trans = "trait", fdr = 0.01)
}
```
...
abline(0,1)
par(xpd = NA)
legend(max(unlist(all.cca.cancor)*1.05), max(unlist(all.boot.cancor)), 
...
for(i in 1:length(tissue.names)){
    ...
    if(length(boot.CCA.results[[i]]) > 1){
        ...
        has.vals <- which(colSums(trait.trans$Composite_Transcripts) != 0)
        ...
        if(!file.exists(qtl.file) || delete_previous){
            has.vals <- which(colSums(trait.trans$Composite_Transcripts) != 0)
            tissue.ctranscript.qtl <- scan1(genoprobs, trait.trans$Composite_Transcripts[,has.vals,drop=FALSE], kinship = K)
            has.vals <- which(colSums(trait.trans$Composite_Traits) != 0)
            tissue.ctrait.qtl <- scan1(genoprobs, trait.trans$Composite_Traits[,has.vals,drop=FALSE], 
                kinship = K)
...
pdf(file.path(results.dir, "all_qtl.transcripts.pdf"), width = 12, height = 5)
    multilod.plot(all.qtl.transcripts[,common.transcript.idx], map, lod.thresh = 0, chr.label.y = 0, 
        ...
        global.max = max(c(unlist(all.qtl.traits), unlist(all.qtl.transcripts))))
dev.off()


pdf(file.path(results.dir, "all_qtl.traits.pdf"), width = 12, height = 5)
    multilod.plot(all.qtl.traits[,common.trait.idx], map, lod.thresh = 0, chr.label.y = 0, 
        chr.label.cex = 1, row.names = colnames(all.qtl.traits)[common.trait.idx],
        mar = c(2, 12, 2, 0),global.color.scale = TRUE, global.min = 4, 
        global.max = max(c(unlist(all.qtl.traits), unlist(all.qtl.transcripts))))
dev.off()

#imageWithTextColorbar(all.qtl.traits, global.color.scale = TRUE, global.min = 4, global.max = max(c(unlist(all.qtl.traits), unlist(all.qtl.transcripts))))
```
...
for(i in 1:length(shared.peaks)){
    ...
    if(length(shared.peaks[[i]]) > 0){
        for(tr in 1:nrow(shared.peaks[[i]])){
            ...
            if(!file.exists(individual.scan.file)){
...
plot.all.together <- function(tissue.mats, tissue.results, tissue.enrich.summary,
    tissue.qtl, tissue.gene.list, tissue.expr, separate.windows = FALSE){
    ...
    rownames(trait.loadings) <- colnames(tissue.mats$Z)
    colnames(trait.loadings) <- trait.description
    has.vals <- which(colSums(transcript.loadings) != 0)
    ...
    for(ct in has.vals){
        ...
        gene.locale <- grep(paste0("Transcript", ct, "_"), names(tissue.gene.list))
        gene.ids <- tissue.gene.list[gene.locale]
        gene.names <- lapply(gene.ids, 
            function(x) gene.table[match(names(x), gene.table[,"gene.id"]),"symbol"])
        #cat(gene.names[[1]], sep = "\n")
        gene.idx <- lapply(gene.ids, function(x) match(names(x), colnames(tissue.expr)))
        gene.expr <- tissue.expr[,unlist(gene.idx),drop=FALSE]
        colnames(gene.expr) <- unlist(gene.names)
        
        if(ncol(gene.expr) <= 5){
            plot.text(paste("Up:", paste(gene.names[[1]], collapse = ", "), 
                "\nDown", paste(gene.names[[2]], collapse = ", ")))
        }
        ...
        if(ncol(gene.expr) > 50){
            ct.loadings <- transcript.loadings[unlist(gene.idx),ct]
            expr.col <- colors.from.values(ct.loadings, split.at.vals = TRUE,
                split.points = 0, col.scale = c("blue", "brown"), grad.dir = "ends")

            ct.decomp <- plot.decomp(t(gene.expr), cols = expr.col)

            #some of the genes cluster with genes that have the opposite loading sign
            #I'm not sure why this is, but the code below shows genes clustered by 
            #gene expression and clustered by loadings, and there's definitely a 
            #difference, so the plotting code is correct, there's just a contrast
            #between the gene expression correlation and the loadings.
            #cor.mat <- cor(gene.expr)
            #cor.order <- hclust(dist(cor.mat))$order
            #ordered.cor <- cor.mat[cor.order, cor.order]
            #ordered.cor <- cor.mat[order(ct.loadings), order(ct.loadings)]
            #pheatmap(ordered.cor, cluster_rows = FALSE, cluster_cols = FALSE)
            ...
            text(x = ct.decomp$u[top.high,1], y = ct.decomp$u[top.high,2], 
                labels = colnames(gene.expr)[top.high], pos = 4, offset = 0.1)
            text(x = ct.decomp$u[top.low,1], y = ct.decomp$u[top.low,2], 
                labels = colnames(gene.expr)[top.low], pos = 4, offset = 0.1)
...
for(i in 1:length(tissue.names)){
    ...
    plot.all.together(tissue.mats = matched.mats[[i]], 
        ...
        tissue.expr = tissue.adj.expr[[i]], separate.windows = is.interactive)
    dev.off()
}
```
...
Fkbp has a cis eQTL on Chr 15 in adipose tissue and skeletal muscle, but not 
in the other two tissues. The tissue where it shows up as correlated with traits
is in islet, where it co-maps with the paired traits to Chr 13. 
