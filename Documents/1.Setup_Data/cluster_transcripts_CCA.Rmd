...
for(i in 1:length(tissue.names)){
    ...
    tissue.results = boot.CCA.results[[i]], percentile = 0)
...
```{r simpEnrich, message = FALSE, error = FALSE, warning = FALSE, eval = FALSE}
...
pdf(file.path(results.dir, "all_qtl.transcripts.pdf"), width = 12, height = 5)
    multilod.plot(all.qtl.transcripts, map, lod.thresh = 0, chr.label.y = 0, 
        chr.label.cex = 1, row.names = colnames(all.qtl.transcripts),
        mar = c(2, 12, 2, 0), global.color.scale = TRUE, global.min = 4, 
        global.max = max(all.qtl.transcripts))
dev.off()

#remove anything with a number greater than 3 - some higher islet traits
#have non-zero loadings, but we don't have paired transcripts for these.
low.ct <- sort(unlist(sapply(1:2, function(x) grep(x, colnames(all.qtl.traits)))))


pdf(file.path(results.dir, "all_qtl.traits.pdf"), width = 12, height = 5)
    multilod.plot(all.qtl.traits[,low.ct], map, lod.thresh = 0, chr.label.y = 0, 
        chr.label.cex = 1, row.names = colnames(all.qtl.traits)[low.ct],
        mar = c(2, 12, 2, 0),global.color.scale = TRUE, global.min = 4, 
        global.max = max(all.qtl.traits))
dev.off()
```
    ...
    for(ct in has.vals){
        ...
        if(length(tissue.enrich.summary[[2]]) > 0){
            ...
            plot.enrichment.wordcloud(tissue.enrich.summary[[2]][[ct.up.locale]], 
                plot.label = names(tissue.enrich.summary[[2]])[ct.up.locale], just.wordcloud = TRUE)
            plot.enrichment.wordcloud(tissue.enrich.summary[[2]][[ct.down.locale]], 
                plot.label = names(tissue.enrich.summary[[2]])[ct.down.locale], just.wordcloud = TRUE)
        }

        #4 - plot expression correlation
        ...
        plot.text(unlist(gene.names))
        }
        #plot.decomp(t(gene.expr))
