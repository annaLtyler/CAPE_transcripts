...
expr.files <- get.files(here("Data", "imputed"), want = "CC", full.names = TRUE)
...
```{r test, eval = FALSE}
#are there any mice with low/normal glucose 
#and high WPIC
gi <- apply(chem.mat[,c(9:12)], 2, as.numeric)
log.gi <- log10(gi+1)
gi.covar <- get_num_covar(chem.mat[,1])
adj.gi <- adjust(log.gi, gi.covar)

sex.col <- gi.covar[,1]+1
diet.treat.pch <- gi.covar[,2]+1
diet.treat.pch[which(gi.covar == 1)] <- diet.treat.pch[which(gi.covar == 1)] + 15
plot(c(adj.gi[,1], adj.gi[,2]), c(adj.gi[,3], adj.gi[,4]),
    xlab = "Glucose", ylab = "Insulin", col = sex.col,
    pch = diet.treat.pch)

Strain <- expr.mouse.info.table[match(chem.mat[,1], expr.mouse.info.table[,1]), "User.Defined.Strain"]

timepoint <- c(rep("Early", nrow(gi)), rep("Late", nrow(gi)))
df <- data.frame("Glucose" = c(gi[,1], gi[,2]), "Insulin" = c(gi[,3], gi[,4]),
    "Sex" = chem.mat[,"Sex"], "Diet" = chem.mat[,"Diet"], 
    "Treatment" = chem.mat[,"Treatment"], "Strain" = Strain, 
    "ID" = chem.mat[,"Mouse.ID"], 
    "Diet_Treat" = apply(chem.mat[,c("Diet", "Treatment")], 1, function(x) paste(x, collapse = "_")),
    "Timepoint" = timepoint)
...
saveWidget(fig, "~/Desktop/Insulin_Glucose.html", selfcontained = TRUE)
```
