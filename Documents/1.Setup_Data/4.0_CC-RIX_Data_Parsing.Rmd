...
The previous analysis found that a composite trait 
...
if(manual.or.CT == "CT"){
    tissue.name <- "Islet" #set tissue from DO analysis
    CT <- 1 #set CT to analyze
...
Gene expression in this analysis was set as `r manual.or.CT`.
...
for(i in 1:length(tissue.expr)){
    expr.mouse.info.table <- t(sapply(rownames(tissue.expr[[i]]), function(x) get_mouse_info(x, manifest, input.type = "User.Inputs")))
    rownames(tissue.expr[[i]]) <- expr.mouse.info.table[,1]
    covar.mouse.info.table <- t(sapply(tissue.annot[[i]][,1], function(x) get_mouse_info(x, manifest, input.type = "User.Inputs")))
...
for(i in 1:length(all.chem.expr.cor)){
    ...
    par(mar = c(10,7,1,1))
    imageWithText(t(all.chem.expr.cor[[i]]), global.color.scale = TRUE, 
...
Gene expression in the adipose tissue is most correlated overall with blood chemistries.
...
for(i in 1:length(tissue.expr)){
    ...
    for(g in 1:nrow(groups)){
        group.locale <- Reduce("intersect", lapply(group.def, 
            function(x) which(matched.chem[[i]][,x] == groups[g,x])))        
        plot.with.model(rankZ(bw.matched.expr[[i]][group.locale]),
...
match_geno_pheno <- function(genoprobs, mouse.pheno.id, manifest){
    ...
    all.pheno.id <- t(sapply(mouse.pheno.id, function(x) get_mouse_info(x, manifest)))
...
region.chr <- "X"; region.start <- 48; region.end <- 50
...
max.allele.expr.effect <- matrix(NA, nrow = length(test.alleles), ncol = length(tissue.expr))
max.allele.bw.effect <- matrix(NA, nrow = length(test.alleles), ncol = 1)
rownames(max.allele.expr.effect) <- rownames(max.allele.bw.effect) <- names(test.alleles)
for(a in 1:length(test.alleles)){
    cat("####", names(test.alleles)[a], "\n")
    #quartz(width = 9, height = 6)
    layout.mat = matrix(c(1:4,0,0), nrow = 2, byrow = TRUE)
    layout(layout.mat)
    ...
    max.allele.expr.effect[a,] <- sapply(max.bw.expr.markers[[2]], function(x) x[1])
    ...
    max.allele.bw.effect[a,] <- sapply(max.bw.markers[[2]], function(x) x[1])
    cat("\n\n")
}
colnames(max.allele.expr.effect) <- names(tissue.expr)
...
ylim <- c(min(max.allele.expr.effect, na.rm = TRUE), max(max.allele.expr.effect, na.rm = TRUE))
par(mfrow = c(2,3))
for(i in 1:length(tissue.expr)){
    allele.order <- order(max.allele.expr.effect[,i])
    barplot(max.allele.expr.effect[allele.order,i], col = CCcolors[allele.order], las = 2,
    ylab = "Maximum Allele Effect on Expression", main = names(tissue.expr)[i])
    abline(h = 0)
}
bw.effect.order <- order(max.allele.bw.effect[,1])
barplot(max.allele.bw.effect[bw.effect.order,1], col = CCcolors[bw.effect.order],
...
max.allele.chem.effect <- matrix(NA, nrow = length(test.alleles), ncol = 10)
rownames(max.allele.chem.effect) <- names(test.alleles)
...
for(a in 1:length(test.alleles)){
    ...
    for(ch in 1:ncol(test.chem[[1]])){
        ...
        max.chem.markers <- test_region_pheno(genoprobs, test.pheno = one.chem, 
            ...
            col = CCcolors[a])
        max.allele.chem.effect[a,ch] <- sapply(max.chem.markers[[2]], function(x) x[1])
    }
    cat("\n\n")
}
colnames(max.allele.chem.effect) <- colnames(test.chem[[1]])
```
...
par(mfrow = c(3,3))
for(i in 1:ncol(max.allele.chem.effect)){
    allele.order <- order(max.allele.chem.effect[,i])
    barplot(max.allele.chem.effect[allele.order,i], 
    col = CCcolors[allele.order], las = 2, ylab = "r", 
    ylim = c(min(max.allele.chem.effect, na.rm = TRUE), max(max.allele.chem.effect, na.rm = TRUE)),
    main = colnames(max.allele.chem.effect)[i])
    abline(h = 0)
}
...
pheatmap(max.allele.chem.effect, cluster_rows = FALSE, cluster_cols = FALSE,
