...
if(use.imputed){
    if(exp.name == "imputed_local"){
        imp.expr.files <- get.files(here("Data", "imputed"), want = "RDS", 
            dont.want = c("adjusted", "local", "CC", "genetic"), full.names = TRUE)
    }
    ...
    for(i in 1:length(tissue.norm)){
        ...
        rownames(tissue.norm[[i]]) <- rownames(expr.data$data$norm)
    }
}else{
...
#test a few to make sure we're not mapping to the end of the chromosome
    ...
    par(mfrow = c(1,2))
    plot(test.measured, map = map, col = "red", main = paste(rnd.name, "Chr", rnd.chr),
        ylim = ylim)
    plot(test.imp, map = map, add = TRUE)
    abline(v = rnd.pos)
