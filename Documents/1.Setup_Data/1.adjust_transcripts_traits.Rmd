...
if(is.na(delete_previous)){
    ...
    #exp.name = "imputed_full"; weight.adjusted = FALSE
...
a minimum amount of expression (unless we are using imputed transcripts). 
Also regress out covariates using the same covariate matrix generated above.

Here we use the raw expression matrix to select transcripts from the rank Z
normalized expression matrix. 

If we are using imputed data, read it in here and overwrite the measured
expression data.
...
if(use.imputed){
    if(exp.name == "imputed_local"){
        adj.expr.file <- here("Data", "imputed", "Adjusted_Expression_Local.RDS")
        imp.expr.files <- get.files(here("Data", "imputed"), want = "RDS", 
            dont.want = c("adjusted", "local", "CC"), full.names = TRUE)
    }
    if(exp.name == "imputed_full"){
        adj.expr.file <- here("Data", "imputed", "Adjusted_Expression_Full.RDS")
        imp.expr.files <- get.files(here("Data", "imputed"), want = c("RDS", "local"), 
            dont.want = c("adjusted", "CC"), full.names = TRUE)
    }
}else{
    if(weight.adjusted){
        adj.expr.file <- here("Data", "Adjusted_Expression_Weight_Adjusted.RDS")
    }else{
        adj.expr.file <- here("Data", "Adjusted_Expression.RDS")
    }
}

#row names are missing in imputed data. Add the rownames
#from the measured DO data.
if(use.imputed){
    #read in imputed data and overwrite tissue.norm
    tissue.norm <- lapply(imp.expr.files, readRDS)
    #add animal names from original expression data
    for(i in 1:length(tissue.norm)){
        tissue.name <- strsplit(basename(imp.expr.files[[i]]), "_")[[1]][2]
        expr.data <- get(paste0("dataset.DO.Cube.", tissue.name))
        rownames(tissue.norm[[i]]) <- rownames(expr.data$data$rz)
    }
}
