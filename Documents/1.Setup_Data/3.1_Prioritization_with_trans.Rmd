...
tissue.name = "Adipose"; network.name = "adipose tissue"
...
adj.traits <- readRDS(here("Data", "Adjusted_Phenotypes.RDS"))
adj.expr <- readRDS(here("Data", "Adjusted_Expression.RDS"))
...
The expression of these genes clusters into two distinct groups.
And the second PC is quite highly negatively correlated with 
the trait.

```{r trans_expr}
tissue.locale <- which(names(adj.expr) == tissue.name)
trans.expr <- adj.expr[[tissue.locale]][,trans.id] 
matched.mats <- get.xz(trans.expr, adj.traits)
trans.decomp <- plot.decomp(matched.mats$X, pc = 4)
