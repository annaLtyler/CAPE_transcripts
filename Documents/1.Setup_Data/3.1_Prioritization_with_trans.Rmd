...
if(!check_bmediatR){
    ...
    devtools::install_github("wesleycrouse/bmediatR", lib = lib.loc, build_vignettes = TRUE)
    library(bmediatR)
}
...
```{r bulk_med, echo = FALSE, message = FALSE, results = "hide"}
...
```{r med_against}
top.genes <- Reduce("union", list(rownames(ordered.med)[1:10], names(which(all.cis.trans.med[,1] > 0.8)), names(which(all.cis.trait.med[,1] > 0.8))))
...
common.genes <- intersect(rownames(fp.mat), rownames(all.cis.trans.med))

plot(-log10(fp.mat[common.genes,1]), all.cis.trans.med[common.genes,1],
    xlab = "Functional Score", ylab = "Post. Prob. of Mediating Trans band",
    ylim = c(0,1), main = "Functional Score v. Trans Band Mediation")
abline(v = 1, h = 0.8)
top.genes <- intersect(which(-log10(fp.mat[common.genes,1]) > 1), which(all.cis.trans.med[common.genes,1] > 0.8))
if(length(top.genes) > 0){
    text(x = -log10(fp.mat[common.genes[top.genes],1]), all.cis.trans.med[common.genes[top.genes],1],
        labels = common.genes[top.genes], pos = 4)
}


plot(-log10(fp.mat[common.genes,1]), all.cis.trait.med[common.genes,1],
    xlab = "Functional Score", ylab = "Post. Prob. of Mediating Trait",
    ylim = c(0,1), main = "Functional Score v. Trait Mediation")
abline(v = 1, h = 0.8)
top.genes <- intersect(which(-log10(fp.mat[common.genes,1]) > 1), which(all.cis.trait.med[common.genes,1] > 0.8))
if(length(top.genes) > 0){
    text(x = -log10(fp.mat[common.genes[top.genes],1]), all.cis.trans.med[common.genes[top.genes],1],
        labels = common.genes[top.genes], pos = 4)
}
