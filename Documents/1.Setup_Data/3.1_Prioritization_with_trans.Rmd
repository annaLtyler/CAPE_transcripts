...
The figure below shows a count of transcripts affected 
by loci across the genome.

```{r cis_trans}

plot.cistrans.table(eqtl.table = as.matrix(eqtl.table), 
    transcript.pos.table = as.matrix(gene.table), map = map)
```
...
plot(-log10(fp.mat[common.genes,1]), expr.med.prob[common.genes],
    xlab = "Functional Score", ylab = "Post. Prob. of Mediating Expression",
    ylim = c(0,1), main = "Functional Score v. Expression Mediation")
abline(v = 1, h = 0.8)
