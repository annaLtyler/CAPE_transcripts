...
if(!file.exists(transcript_loading_file) || delete_previous){
    ...
    for(tx in 1:length(scaled.expr)){
        ...
        model_scores[[tx]] <- high_dim_med(causal.matrix = Kg_cent[[tx]], 
            ...
            num.iterations = 5, verbose = is.interactive, kernel.c = TRUE, 
    ...
    for(tx in 1:max.tx){
        ...
        for(p in 1:num.perm){
            ...
            perm_cor <- cor(high_dim_med(causal.matrix = Kg_cent[[tx]], 
                ...
                num.iterations = 3, verbose = TRUE, kernel.c = TRUE, kernel.m = TRUE, 
                kernel.o = TRUE))
            #diff_from_ideal(perm_cor)

            perm_path[p] <- abs(perm_cor[1,3])
        }
...
for(tx in 1:max.tx){
    ...
    tissue.path.coef <- tissue.cor[3,1]
    xmax <- max(c(all.perms[[tx]], tissue.path.coef))
    xlim <- get_plot_bounds(0, xmax)

    hist(all.perms[[tx]], xlim = xlim, breaks = 20, main = "Path Coef Null",
        xlab = "Path Coef")
...
for(tx in 1:length(tissue.names)){
    ...
    cor_diff <- diff_from_ideal(curr_cor)
...
if(!file.exists(enrich.file) || delete_previous){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        top.hum.id <- mouse_to_human_entrez(top.id)
        ...
        bottom.hum.id <- mouse_to_human_entrez(bottom.id)
