...
#This function subsets an enrichment matrix to only terms with
#a minimum number of individuals (min.ind)
subset_enrichment <- function(individual.enrichments, max.term.size = 5000,
    max.char = 100, enrich.sig.thresh = 1e3, min.ind = 20){
    ...
    #count the number of individuals with each term
    num.ind <- apply(sig.enrich.mat, 1, function(x) length(which(x > 0)))
    sub.mat <- sig.enrich.mat[which(num.ind > min.ind),]
    #pheatmap(sub.mat, show_colnames = FALSE)
...
if(mediation.text == "complete_mediation"){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        #clustered_down_enrich <- summarize_enrichment(individual.enrichments = ind.down.enrich, 
        ...
        clustered_down_enrich <- subset_enrichment(ind.down.enrich, 
            max.term.size = 5000, max.char = 100, min.ind = 20)
        ...
        pdf(file.path(results.dir, paste0("Clustered_Enrichment_", 
            ...
            imageWithText(clustered_down_enrich[row_order, mean.order], 
                use.pheatmap.colors = TRUE, show.text = FALSE, col.names = NULL,
                row.text.cex = 0.7)
...
if(mediation.text == "complete_mediation"){
    for(tx in 1:length(tissue.names)){
        ...
        pdf(file.path(paste("LOD_vs_Loading_", tissue.names[tx], ".pdf")), width = 10, height = 8)
