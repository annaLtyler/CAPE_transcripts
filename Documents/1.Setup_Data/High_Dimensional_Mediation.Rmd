...
if(mediation.text == "complete_mediation"){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        top.hum.id <- mouse_to_human_entrez(names(top.pressure), hum.mus.ortho)
        bottom.pressure <- sort(pop.pressure[[tx]], decreasing = FALSE)[1:200]
        bottom.hum.id <- mouse_to_human_entrez(names(bottom.pressure), hum.mus.ortho)
...
diet.idx <- which(manifest[,"Diet"] == "44% fat + fiber")
treatment.idx <- which(manifest[,"Treatment"] == "None")
...
for(st in 1:nrow(cc.geno[[1]])){
    ...
    weight.idx <- which(cc.bw[,1] %in% mouse.ids)
    strain.weight[[st]] <- cc.bw[weight.idx,2]
}
...
for(tx in 1:length(tissue.names)){
    ...
    plot.with.model(mean.weight[f.weight.locale], cc.mean[f.pressure.locale], main = "Female")
    plot.with.model(mean.weight[m.weight.locale], cc.mean[m.pressure.locale], main = "Male")

    df <- data.frame("weight" = mean.weight, "sex" = sex, 
        "transcriptomic_pressure" = cc.mean)
    library(ggplot2)
    p <- qplot(weight, transcriptomic_pressure, data = df, 
        geom = c("point", "smooth"), facets = ~sex)
    p    
}
```
