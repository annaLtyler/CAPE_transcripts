...
all.packages <- c("pheatmap", "qtl2", "gprofiler2", "Matrix", "RGCCA", "bnstruct")
...
```{r flip_scores}
#sometimes one set of scores gets flipped giving us
#negative correlations in the scores. This function 
#flips everything around to be positively correlated.

flip_scores <- function(med_scores){
    curr_cor = cor(med_scores)
    neg_cor <- which(curr_cor < 0, arr.ind = TRUE)
    #if there are any that are negative, flup them arround
    if(length(neg_cor) > 1){
        #which has the most negative correlations
        max.neg <- as.numeric(names(which.max(table(as.vector(neg_cor)))))
        #flip the scores
        med_scores[,max.neg] <- med_scores[,max.neg]*-1
    }
    return(med_scores)
}
...
if(!file.exists(transcript_loading_file) || delete_previous){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        model_scores[[tx]] <- high_dim_med(causal.matrix = Kg_cent[[tx]], 
        ...
        #pairs(model_scores[[tx]])
        if(any(cor(model_scores[[tx]]) < 0)){
            flipped_result <- flip_scores(model_scores[[tx]])
            #pairs(flipped_result)
            model_scores[[tx]] <- flipped_result
        }
    ...
    #med.coef <- t(sapply(1:4, function(tx) path_coef(cor(model_scores[[tx]]))))
...
if(!file.exists(perm.file) || delete_previous){
    ...
    for(tx in 1:max.tx){
        ...
        perm_path <- matrix(NA, nrow = num.perm, ncol = 3)
        colnames(perm_path) <- c("path_coef", "obs_cor", "diff")
        for(p in 1:num.perm){
            ...
            perm_scores <- high_dim_med(causal.matrix = Kg_cent[[tx]], 
                ...
                min.weight.diff = 1e-5, max.iter = 15, verbose = is.interactive, 
                kernel.c = TRUE, kernel.m = TRUE, kernel.o = TRUE)
            
            if(any(cor(model_scores[[tx]]) < 0)){
                flipped_result <- flip_scores(perm_scores)
                #pairs(flipped_result)
                perm_scores <- flipped_result
            }

            perm_cor <- cor(perm_scores)
            perm_path[p,] <- path_coef(perm_cor)
        }
...
for(tx in 1:max.tx){
    tissue.cor <- cor(model_scores[[tx]])
    tissue.path.coef <- path_coef(tissue.cor)
    xmax <- max(c(all.perms[[tx]][,1], tissue.path.coef[1]))
    xlim <- get_plot_bounds(0, xmax)

    ylim <- c(0, max(abs(c(unlist(all.perms[[tx]]), tissue.path.coef))))
    boxplot(abs(all.perms[[tx]]), ylim = ylim, col = categorical_pal(3))
    segments(x0 = segment.region(0.75, 2.75, 3, "ends"), 
            x1 = segment.region(1.25, 3.25, 3, "ends"),
            y0 = tissue.path.coef, col = categorical_pal(3), lwd = 3)

    stripchart(-log10(abs(all.perms[[tx]][,3])), method = "jitter", pch = 16, 
        col = "gray", vertical = TRUE, 
        main = "-log10[cor(x,y) - path coef.]")
    segments(x0 = 0.9, x1 = 1.1, y0 = -log10(abs(tissue.path.coef[3])), col = "red", lwd = 3)
    text(x = 0.89, y = -log10(abs(tissue.path.coef[3])), labels = "Observed", adj = 1)

    plot(all.perms[[tx]][,1:2], ylim = ylim, xlim = ylim, xlab = "cor(x,m)*cor(m,y)",
        ylab = "cor(x,y)", col = "gray", pch = 16)
    points(tissue.path.coef[1], tissue.path.coef[2], col = "red", pch = 16)
    abline(0,1)
}
```
...
for(tx in 1:length(tissue.names)){
    ...
    #if there are any that are negative, flup them arround
    if(any(curr_cor < 0)){
        flipped.results <- flip_scores(curr_scores, )
    }
        #which has the most negative correlations
        max.neg <- as.numeric(names(which.max(table(as.vector(neg_cor)))))
        #flip the scores
        curr_scores[,max.neg] <- curr_scores[,max.neg]*-1
        #and replace in the model_scores object
        model_scores[[tx]] <- curr_scores

        #recalculate correlations
        curr_cor = cor(curr_scores)

        #also flip the calculated loadings if required
        if(max.neg == 2){
            transcript_loadings[[tx]] <- transcript_loadings[[tx]]*-1
        }
        if(max.neg == 3){
            trait_loadings[[tx]] <- trait_loadings[[tx]]*-1
        }
        
        #save corrected loadings in case we want to use them in
        #a different workflow downstream
        saveRDS(transcript_loadings, transcript_loading_file)
        saveRDS(model_scores, model_file)
    }
    cor_diff <- path_coef(curr_cor)[3]
    #cor_diff <- abs(curr_cor[1, 3] - curr_cor[1,2] * curr_cor[2,3])
    pairs(curr_scores, main = paste(tissue.names[tx], "\nCorrelation Diff:", round(cor_diff, 3)))
    cat("\n\n")
}
