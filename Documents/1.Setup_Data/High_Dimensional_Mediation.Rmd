...
for(tx in 1:max.tx){
    ...
    hist(all.perms[[tx]], xlim = c(xmin, xmax), main = "Permutations")
...
```{r plot_enrichment, results = "asis", fig.width = 10, fig.height = 5, warning = FALSE, messag = FALSE}
for(tx in 1:length(tissue.names)){
    cat("###", tissue.names[tx], "{.tabset .tabset-fade .tabset-pills}\n")

    cat("#### Positive Loadings\n")
    par(mfrow = c(1,2))
    plot.enrichment.wordcloud(all.enrich[[tx]]$top, order.by = "p_value",
...
summarize_enrichment <- function(individual.enrichments, max.term.size = 5000,
    ...
    missing.names <- setdiff(colnames(expr.by.load), colnames(enrich.mat)) 
    ...
    clustered_enrich <- summarize_go_mat(go_term_mat = enrich.mat, 
        go_term_sim = ind.down.sim, min.cl = min.cl, max.cl = max.cl,
...
```{r polygenic_risk_score, results = "asis", warning = FALSE, message = FALSE}
if(mediation.text == "complete_mediation"){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        pop.pressure <- tx.r2[common.transcripts]*trans.load[common.transcripts,1]
        #hist(pop.pressure, breaks = 100)
        pop.neg.pressure <- gost(names(sort(pop.pressure))[1:enrichment.num], 
            organism = "mmusculus")
        pop.pos.pressure <- gost(names(sort(pop.pressure, decreasing = TRUE))[1:enrichment.num], 
            organism = "mmusculus")

        cat("#### Population-Level Transcriptional Pressure Enrichments\n")
        par(mfrow = c(1,2))
        plot.enrichment.wordcloud(pop.neg.pressure, 
            plot.label = "Transcripts with population-level negative pressure")
        plot.enrichment.wordcloud(pop.pos.pressure, 
        ...
        if(!file.exists(ind.down.enrichment.file)){
            ind.down.enrich <- apply(expr.by.load, 2, 
                function(x) gost(rownames(expr.by.load)[order(x)[1:enrichment.num]], 
                organism = "mmusculus"))
        ...
        if(!file.exists(ind.up.enrichment.file)){
            ind.up.enrich <- apply(expr.by.load, 2, 
                function(x) gost(rownames(expr.by.load)[order(x, decreasing = TRUE)[1:enrichment.num]], 
                organism = "mmusculus"))
        ...
        clustered_down_enrich <- summarize_enrichment(ind.down.enrich, 
            max.term.size = 5000, max.char = 100, enrich.sig.thresh = 1e3, 
            top.n.terms = 5, min.cl = 2, max.cl = 20)

        pdf(file.path(results.dir, paste0("Clustered_Down_Enrichment_", 
            tissue.names[tx], ".pdf")), width = 18, height = 6)
        pheatmap(clustered_down_enrich[,colnames(expr.by.load)[mean.order]], 
            show_colnames = FALSE, cluster_cols = FALSE)
        dev.off()

        clustered_up_enrich <- summarize_enrichment(ind.up.enrich, 
            max.term.size = 5000, max.char = 100, enrich.sig.thresh = 1e3, 
            top.n.terms = 5, min.cl = 2, max.cl = 20)

        pdf(file.path(results.dir, paste0("Clustered_Up_Enrichment_", 
            tissue.names[tx], ".pdf")), width = 18, height = 6)
        pheatmap(clustered_up_enrich[,colnames(expr.by.load)[mean.order]], 
            show_colnames = FALSE, cluster_cols = FALSE)
        dev.off()

        #a <- t(apply(clustered_down_enrich[,colnames(expr.by.load)], 1, 
        #    function(x) plot.with.model(x, trait.pc[colnames(expr.by.load),1], 
        #    plot.results = FALSE)))
        #plot.with.model(clustered_down_enrich[24,colnames(expr.by.load)], trait.pc[colnames(expr.by.load),1])


        #a <- t(apply(clustered_down_terms[,colnames(expr.by.load)], 1, 
        #    function(x) plot.with.model(x, trait.pc[colnames(expr.by.load),1], 
        #    plot.results = FALSE)))

        #test <- rbind(clustered_up_terms[,colnames(expr.by.load)[mean.order]], 
        #    clustered_down_terms[,colnames(expr.by.load)[mean.order]])
        #pheatmap(test[,mean.order])

        cat("#### Transcriptional Pressure Distributions\n\n")
        
        #bin transcriptional pressure so we can see all of them.
        bins <- segment_region(min(expr.by.load), max(expr.by.load), 100, "ends")
        binned.pressure <- apply(expr.by.load, 2, function(x) hist(x, plot = FALSE, breaks = bins))
        ...
        par(mar = c(0,4,2,4))
        barplot(colMeans(expr.by.load[,mean.order]), names = NA, 
        ...
        loading.col <- colors.from.values(ind.score, use.pheatmap.colors = TRUE)
        plot.decomp(t(large.mat), cols = loading.col)
        row.order <- hclust(dist(large.mat))$order
        col.order <- hclust(dist(t(large.mat)))$order
        imageWithText(large.mat[row.order, col.order], show.text = FALSE)
        ind.order <- order(ind.score)

        pheatmap(large.mat[ind.order,], cluster_rows = FALSE, show_rownames = FALSE, show_colnames = FALSE)
