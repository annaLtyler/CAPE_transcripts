...
if(!file.exists(transcript_loading_file) || delete_previous){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        if(mediation.text == "complete_mediation"){
            ...
            outcome.kernel = Kp
            transcript.col <- "Mediator"
            trait.col <- "Outcome"
        }else{
            mediator.kernel = Kp
            outcome.kernel = Kt[[tx]]
            transcript.col = "Outcome"
            trait.col <- "Mediator"
        }
...
if(!file.exists(perm.file) || delete_previous){
    ...
    for(tx in 1:max.tx){
        #assign the proper mediating and outcome matrices
        if(mediation.text == "complete_mediation"){
            mediator.kernel = Kt[[tx]]
            outcome.kernel = Kp
        }else{
...
for(tx in 1:max.tx){
    ...
    ylim <- c(min(c(unlist(all.perms[[tx]]), tissue.path.coef)), 
        max(c(unlist(all.perms[[tx]]), tissue.path.coef)))
    boxplot(all.perms[[tx]], ylim = ylim, col = categorical_pal(3))
