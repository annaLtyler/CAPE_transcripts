...
x.bins <- 100
y.bins <- 30
weight.vals <- segment.region(min(weight.diff), max(weight.diff), x.bins, "ends")
binned.weight <- bin.vector(weight.diff, weight.vals)
u_weight <- unique(binned.weight)
weight.idx <- lapply(u_weight, function(x) which(binned.weight == x))
weight.list <- lapply(weight.idx, function(x) binned.weight[x])
weight.col <- lapply(weight.idx, function(x) sex.col[x])
max.len <- max(sapply(weight.list, length))
yvals <- segment.region(0, 1, max.len, "ends")
pdf("~/Desktop/test.pdf", height = 5, width = 5)
plot.new()
plot.window(xlim = c(min(weight.diff), max(weight.diff)), ylim = c(0,1))
for(i in 1:length(u_weight)){
    points(weight.list[[i]], yvals[1:length(weight.list[[i]])], 
        col = weight.col[[i]], pch = 16)
}
abline(v = 1)
axis(1);axis(2)
mtext("Proportion Weight Change", side = 1, line = 2.5)
...
for(tx in 1:max.tx){
    ...
    hist(all.perms[[tx]], xlim = c(xmin, xmax), main = "Permutations", xlab = "")
...
if(mediation.text == "complete_mediation"){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        mean.order <- order(colMeans(transcript.pressure))
        ...
        pdf(file.path(results.dir, paste0("Clustered_Down_Enrichment_", 
            ...
            pheatmap(clustered_down_enrich[,colnames(transcript.pressure)[mean.order]], 
                show_colnames = FALSE, cluster_cols = FALSE)
            row_order <- hclust(dist(clustered_down_enrich))$order
            col_order <- hclust(dist(t(clustered_down_enrich)))$order
            layout(matrix(c(1,2), ncol = 1), heights = c(0.5, 1))
            par(mar = c(0,25,2,2))
            barplot(ind.score[col_order], names = NA, ylab = "Predicted Score")
            par(mar = c(0,25,0,2))
            imageWithText(clustered_down_enrich[row_order, col_order], 
        ...
        pdf(file.path(results.dir, paste0("Clustered_Up_Enrichment_", 
            tissue.names[tx], ".pdf")), width = 18, height = 6)
            row_order <- hclust(dist(clustered_up_enrich))$order
            layout(matrix(c(1,2), ncol = 1), heights = c(0.5, 1))
            par(mar = c(0,30,2,0))
            barplot(ind.score[mean.order], names = NA, ylab = "Predicted Score")
            par(mar = c(0,30,0,0))
            imageWithText(clustered_up_enrich[row_order,colnames(transcript.pressure)[mean.order]], 
                col.names = NULL, show.text = FALSE, use.pheatmap.colors = TRUE)
            col_order <- hclust(dist(t(clustered_up_enrich)))$order
            layout(matrix(c(1,2), ncol = 1), heights = c(0.5, 1))
            par(mar = c(0,30,2,0))
            barplot(ind.score[col_order], names = NA, ylab = "Predicted Score")
            par(mar = c(0,30,0,0))
            imageWithText(clustered_up_enrich[row_order, col_order], 
                use.pheatmap.colors = TRUE, show.text = FALSE, col.names = NULL,
                row.text.cex = 0.7)
        dev.off()
        ...
        par(mfrow = c(1,2))
        pop.pressure.transcript.col <- colors.from.values(pop.pressure, 
...
```{r lod_v_loading, results = "asis", fig.height = 5, fig.width = 5}
if(mediation.text == "complete_mediation"){
    for(tx in 1:length(tissue.names)){
        cat("###", tissue.names[tx], "\n")
        eqtl.table <- get(data.set.names[[tx]])$lod.peaks$additive
        ...
        pressure.col <- colors.from.values(pop.pressure[gene.id], use.pheatmap.colors = TRUE)
        #pdf("~/Desktop/test.pdf")
        #plot in the order of population-level transcriptomic pressure,
        #so we can see the outliers more easily
        pressure.order <- order(abs(pop.pressure[gene.id]), decreasing = FALSE, na.last = FALSE)
        xV <- max.lod[pressure.order] 
        yV <- abs(transcript_loadings[[tx]][,1])[pressure.order]
        ...
        text(tail(xV, num.label), tail(yV, num.label), 
            labels = names(tail(yV, num.label)), pos = 4)
