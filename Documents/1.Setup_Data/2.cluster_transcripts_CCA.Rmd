...
for(i in 1:n.expr.pc){
    ...
    xlab = paste("Expression PC", i), ylab = paste("Trait PC", j), report = "cor.test")
...
global.max <- max(c(sapply(tissue.pc.cor.mats, function(x) max(abs(x))), max(overall.cor.mat)))
global.min <- 0
...
describe_results <- function(tissue.mats, tissue.results, filename = NULL){
    ...
    if(is.null(tissue.results)){
        ...
        if(length(has.transcript) > 1){
            ...
            cat("#### Trait Network\n")
            layout(matrix(c(1,1,2,0), ncol = 2, byrow = FALSE), widths = c(1, 0.2))
