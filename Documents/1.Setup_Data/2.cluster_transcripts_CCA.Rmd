...
cis.lod.file <- here("Results", "CCA_clusters", "imputed", "cis.lod.scores.RDS")
...
for(i in 1:length(matched.mats)){
    ...
    for(tr in 1:length(non.zero)){
        ...
        cis.marker <- find_marker(map, gene.table[gene.idx,"chr"], as.numeric(gene.table[gene.idx,"start"]))
        #scan measured transcript expression
        measured.qtl <- scan1(genoprobs[,gene.table[gene.idx,"chr"]], trans.expr) 
        imp.qtl <- scan1(genoprobs[,gene.table[gene.idx,"chr"]], imp.expr)
        med.qtl.expr <- scan1(genoprobs[,gene.table[gene.idx,"chr"]], trans.expr, addcovar = imp.expr)
        med.qtl.marker <- scan1(genoprobs[,gene.table[gene.idx,"chr"]], trans.expr, addcovar = genoprobs[[gene.table[gene.idx,"chr"]]][,,cis.marker])
        ...
        if(plot.results){
            ...
            plot(imp.qtl, map = map, col = "red", add = TRUE);abline(v = as.numeric(gene.table[gene.idx,"start"]))
