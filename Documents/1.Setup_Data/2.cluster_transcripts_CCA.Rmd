...
nperms = 10 #number of permutations to do in penalized CCA to determine optimal penalties
...
perc.drop.thresh = 0.75 #fraction of full LOD score mediated LOD score needs to be to be scanned further
```
...
for(i in 1:length(matched.mats)){
    ...
    par(mfrow = c(2,2))
    imageWithText(perm.results$Z, col.scale = "red", main = "Z statistics", col.text.shift = 0.07,
    row.text.shift = 0.07)
    imageWithText(perm.results$Cor, col.scale = "blue", main = "Correlation", col.text.shift = 0.07,
    row.text.shift = 0.07)
...
sig_loadings <- function(estimate.mat, fdr = 0.05, plot.sample = FALSE){ 
    ...
    all.p <- apply(estimate.mat[has.vals,], 1, function(x) t.test(x, mu = 0)$p.value)
    ...
    if(plot.sample){
        non.zero.sample <- sample(has.vals[sig.tests], 20)
        zero.sample  <- sample(setdiff(1:nrow(estimate.mat), has.vals[sig.tests]), 20)
...
for(i in 1:length(shared.peaks)){
    ...
    if(length(shared.peaks[[i]]) > 0){
        ...
        for(tr in 1:nrow(shared.peaks[[i]])){
            ...
            peak.marker <- which(map[[chr]] == pos.trait.qtl)
            ...
            if(!file.exists(mediation.scan.file)){
            ...
            #if the percent drop is greater than 50% run the blups to visualize the 
            #allele effects
            if(perc.drop > perc.drop.thresh){
                ...
                if(!file.exists(individual.scan.file) || delete_previous){
                    ...
                    #map the transcript
                    transcript.lod <- scan1(genoprobs[,chr], trans[,trait,drop=FALSE], kinship = K[[chr]])
                    #find_peaks(transcript.lod, map = map, threshold = 5, prob = 0.95)
                    #transcript.coef <- scan1coef(genoprobs[,chr], trans[,trait,drop=FALSE], kinship = K[[chr]])
                    transcript.coef <- scan1blup(genoprobs[,chr], trans[,trait,drop=FALSE], kinship = K[[chr]])
            
                    #map the trait too
                    trait.lod <- scan1(genoprobs[,chr], ctrait[,trait,drop=FALSE])
                    #find_peaks(trait.lod, map = map, threshold = 5, prob = 0.95)
                    #trait.coef <- scan1coef(genoprobs[,chr], ctrait[,trait,drop=FALSE])
...
#tissue.name <- "Islet"; CT = 1; Chr = "X"
tissue.name <- "Adipose"; CT = 1; Chr = "X"
#tissue.name = "Liver"; CT = 1; Chr = 17
#tissue.name = "Islet"; CT = 18; Chr = 11
#tissue.name <- "Adipose"; CT = 19; Chr = 11
#tissue.name <- "Liver"; CT = 5; Chr = 4
#tissue.name <- "Liver"; CT = 5; Chr = 8
...
trait.qtl <- scan1(genoprobs[,Chr], trait.trans$Composite_Traits[,CT,drop=FALSE], kinship = K[[chr.num]])
transcript.qtl <- scan1(genoprobs[,Chr], trait.trans$Composite_Transcripts[,CT,drop=FALSE], kinship = K[[chr.num]])
transcript.peaks <- find_peaks(transcript.qtl, map = map, threshold = 5, prob = 0.95)
trait.peaks <- find_peaks(trait.qtl, map = map, threshold = 5, prob = 0.95)

quartz(width = 10, height = 6);par(mfrow = c(2,1))
plot(trait.qtl, map = map, main = "CTrait")
abline(v = trait.peaks[1,"pos"])
plot(transcript.qtl, map = map, main = "CTranscript")
abline(v = transcript.peaks[1,"pos"])

trait.peak.lod <- trait.peaks[1,"lod"]
trait.peak.pos <- trait.peaks[1,"pos"]
transcript.peak.lod <- transcript.peaks[1,"lod"]
transcript.peak.pos <- transcript.peaks[1,"pos"]
...
plot(trait.qtl, map = map)
plot(med.qtl, map = map, add = TRUE, col = "red")

#Use Mouse Mine to make a list of positional candidates for the region identified above
...
#write.table(full.table[loading.order,], "~/Desktop/Adipose_CT1_transcripts.txt", quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
#length(transcripts)
...
locus.genes <- gene.info[[tissue.name]][intersect(mb.locale, chr.locale),]


#Use Mouse Mine to make a list of affected transcripts
write.table(transcripts, "~/Desktop/transcripts.txt", quote = FALSE, sep = "\t", 
...
overlap.genes <- intersect(transcript.names, locus.genes)
gene.name <- locus.genes[,"symbol"]
...
barplot_with_num(sort(round(mediated.lod[,1], 2)), main = "Mediated Trait Peaks", las = 2)
abline(h = trait.peak.lod)
barplot_with_num(sort(round(mediated.lod[,2], 2)), main = "Mediated Transcript Peaks", las = 2)
abline(h = transcript.peak.lod)

plot(mediated.lod, pch = 16, xlim = c(min(mediated.lod[,1]), max(mediated.lod[,1])*1.1))
text(mediated.lod[,1], mediated.lod[,2], labels = gene.names, pos = 4)
abline(v = trait.peak.lod, h = transcript.peak.lod) 

candidate.name = "Sigmar1"

#show heatmap of individual gene expression along with trait values
ct.loadings <- boot.CCA.results[[tissue.locale]]$u[,CT,drop=FALSE]
most.pos <- order(ct.loadings, decreasing = TRUE)[1:10]
most.neg <- order(ct.loadings, decreasing = FALSE)[1:10]
cand.id <- gene.info[[tissue.name]][match(candidate.name, gene.info[[tissue.name]][,"symbol"]),1]
cand.pos <- which(colnames(matched.mats[[tissue.locale]]$X) == cand.id)
ct.expr <- matched.mats[[tissue.locale]]$X[,c(most.pos, most.neg, cand.pos),drop=FALSE];show.gene.names = TRUE

#use just SH3 domain containing genes
#sh3.genes <- read.delim("~/Downloads/SH3_genes.tsv", stringsAsFactors = FALSE)
#sh3.gene.id <- gene.info[[tissue.name]][match(sh3.genes[,2], gene.info[[tissue.name]][,"symbol"]),1]
#ct.expr <- matched.mats[[tissue.locale]]$X[,sh3.gene.id,drop=FALSE];show.gene.names = TRUE

#or use all non-zero
ct.expr <- matched.mats[[tissue.locale]]$X[,non.zero.loadings,drop=FALSE];show.gene.names = FALSE

colnames(ct.expr) <- gene.info[[tissue.name]][match(colnames(ct.expr), gene.info[[tissue.name]][,1]),"symbol"]

trait.df <- data.frame(trait.trans$Composite_Traits[,CT,drop=FALSE])
colnames(trait.df) <- "Composite_Trait1"

ctrait.order <- order(trait.df[,1], decreasing = TRUE)

#make the heatmap just to get the order of the columns
test <- pheatmap(ct.expr[ctrait.order,], show_rownames = FALSE, 
    show_colnames = show.gene.names, cluster_rows = FALSE, cluster_cols = TRUE, 
    annotation_row = trait.df, scale = "column")
ctranscript.order <- test$tree_col$order

#plot without dendrogram
png("~/Desktop/CT_transcripts_expression.png", width = 10, height = 4, units = "in", res = 200)
pheatmap(t(ct.expr[ctrait.order,ctranscript.order]), show_colnames = FALSE, 
    show_rownames = show.gene.names, cluster_rows = FALSE, cluster_cols = FALSE, 
    annotation_col = trait.df, scale = "row")
dev.off()


#use row ordering
pheatmap(ct.expr[ctrait.order,ctranscript.order], show_rownames = FALSE, 
    show_colnames = show.gene.names, cluster_rows = TRUE, cluster_cols = TRUE, 
    annotation_row = trait.df, scale = "column")



#big.mat <- cbind(all.expr, trait.trans$Composite_Traits[,CT,drop=FALSE], trait.trans$Composite_Transcripts[,CT,drop=FALSE])
#pheatmap(cor(big.mat), display_numbers = TRUE)
#pairs(big.mat)
...
barplot_with_num(sort(round(eqtl.lod, 2)), las = 2)

#pairs(all.expr)
#bulk.adj.trait <- adjust(trait.trans$Composite_Traits[,CT,drop=FALSE], all.expr)
#bulk.test.med <- scan1(genoprobs[,Chr], bulk.adj.trait, kinship = K[[chr.num]])
#plot(bulk.test.med, map = map)
#get_LOD(bulk.test.med, map = map, chr = Chr, pos = trait.peak.pos)
