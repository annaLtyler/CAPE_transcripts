...
var.decomp <- lapply(tissue.names[1:3], function(x) readRDS(here("Data", "imputed", 
    paste0("DO_", x, "_imputed_variance_decomposition.RDS"))))
names(var.decomp) <- tissue.names[1:3]
...
## Combining Results Across Tissues

If we are interested in a single trait, HOMA_B for example,
can we tell which transcripts in which traits are most likely
associated with it?

```{r combining, eval = FALSE}

all.trait.loadings <- all.transcript.loadings <- vector(mode = "list", length = length(tissue.names))
names(all.trait.loadings) <- names(all.transcript.loadings) <- tissue.names
for(tx in 1:length(tissue.names)){
    tissue.results <- boot.CCA.results[[tx]]
    tissue.mats <- matched.mats[[tx]]
    trait.loadings <- tissue.results$v
    trait.description <- get_trait_description(tissue.mats, tissue.results, 
    short_version = TRUE)
    rownames(trait.loadings) <- colnames(matched.mats[[tx]]$Z)
    colnames(trait.loadings) <- trait.description
    
    transcript.loadings <- tissue.results$u
    rownames(transcript.loadings) <- colnames(matched.mats[[tx]]$X)

    all.trait.loadings[[tx]] <- trait.loadings
    all.transcript.loadings[[tx]] <- transcript.loadings

}

#get all loadings for particular trait
#colnames(cca.pheno)
trait.name <- colnames(cca.pheno)
#trait.name <- c("HOMA_B_0min", "HOMA_IR_0min")
ind.trait.loadings <- t(Reduce("cbind", lapply(all.trait.loadings, function(x) x[trait.name,,drop=FALSE])))
rownames(ind.trait.loadings) <- paste(rep(tissue.names, each = length(trait.name)), gsub("Composite_Trait", "", rownames(ind.trait.loadings)), sep = "_")

pdf("~/Desktop/all_loadings.pdf", width = 12, height = 6)
pheatmap(abs(t(ind.trait.loadings)), cluster_cols = FALSE)
dev.off()



common.transcripts <- Reduce("intersect", lapply(all.transcript.loadings, rownames))
ind.transcript.loadings <- Reduce("cbind", lapply(all.transcript.loadings, function(x) x[common.transcripts,1,drop=FALSE]))
#colnames(ind.transcript.loadings) <- tissue.names
colnames(ind.transcript.loadings) <- paste(rep(tissue.names, each = length(trait.name)), gsub("Composite_Transcript", "", colnames(ind.trait.loadings)), sep = "_")
row.vals <- which(rowSums(ind.transcript.loadings) != 0)
col.vals <- which(colSums(ind.transcript.loadings) != 0)

pdf("~/Desktop/all_transcript_loadings2.pdf", width = 4, height = 30)
pheatmap(abs(ind.transcript.loadings[row.vals,col.vals]), 
    cluster_cols = FALSE, show_rownames = FALSE)
dev.off()

ind.list <- apply(ind.transcript.loadings, 2, function(x) which(x != 0))
plotVenn(ind.list)
#core transcript network: connect transcripts to tissue node by product of 
#trait weight and transcript weight
trait.name <- "HOMA_B_0min"
trans.by.trait <- lapply(1:length(tissue.names), function(x) all.trait.loadings[[x]] %*% t(all.transcript.loadings[[x]]))
common.trans <- Reduce("intersect", lapply(trans.by.trait, colnames))
one.trans.by.trait <- sapply(trans.by.trait, function(x) x[trait.name,common.trans])
colnames(one.trans.by.trait) <- tissue.names
#hist(trans.by.trait[trait.name,])

pdf("~/Desktop/homa_b_transcripts.pdf", width = 4, height = 20)
pheatmap(one.trans.by.trait, show_rownames = FALSE)
dev.off()
...
pdf(paste0("~/Desktop/", exp.name, "_cor.pdf"), width = 8, height = 7)
par(mar = c(8, 4, 4, 4))
for(i in 1:length(trans.cor)){
    boxplot(t(trans.cor[[i]]), las = 2, main = tissue.names[i], ylim = c(-0.6, 0.6))
}
dev.off()

tx = 1
ordered.cor <- apply(t(trans.cor[[tx]]), 2, function(x) sort(abs(x), decreasing = TRUE))
head(ordered.cor)
ordered.names <- apply(t(trans.cor[[tx]]), 2, function(x) gene.info[[tx]][match(colnames(trans.cor[[tx]])[order(abs(x), decreasing = TRUE)], gene.info[[tx]][,1]),"symbol"])
head(ordered.names)

apply(ordered.names, 2, function(x) which(x == "Slfn3"))

gene.name = "Slfn3";trait.name = "HOMA_B_0min"
gene.name <- "Gm11427"; trait.name = "Ins_tAUC"
gene.id <- gene.info[[tx]][which(gene.info[[tx]][,"symbol"] == gene.name),1]
plot.with.model(matched.mats[[tx]]$X[,gene.id], matched.mats[[tx]]$Z[,trait.name],
xlab = gene.name, ylab = trait.name)

#used transcript var explained
for(tx in 1:length(tissue.names)){
    tissue.transcripts <- boot.CCA.results[[tx]]$u
    rownames(tissue.transcripts) <- colnames(matched.mats[[tx]]$X)
    used.var.exp <- apply(tissue.transcripts, 2, function(x) var.decomp[[tx]][rownames(tissue.transcripts)[which(x != 0)],])
    plot.grouped.boxes(used.var.exp, type = "matrix")
}
```
