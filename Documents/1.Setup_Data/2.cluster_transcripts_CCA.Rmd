...
for(i in 1:length(matched.mats)){
    ...
    tissue.trait.penalty[[i]] <- perm_best_results(perm.results, pval.thresh = pval.thresh, 
        cor.diff.thresh = cor.diff.thresh, col.text.shift = 0.1, row.text.shift = 0.1,
        return.top.only = TRUE)
    cat("\n\n")
}
    ...
    if(length(all.lod.drop[[i]]) > 0){
        for(tr in 1:nrow(all.lod.drop[[i]])){
            ...
            if(lod.cut > perc.drop.thresh){
                ...
                transcript.peaks <- find_peaks(transcript.qtl, map = map, threshold = lodthresh, prob = 0.95)
                trait.peaks <- find_peaks(trait.qtl, map = map, threshold = lodthresh, prob = 0.95)
                ...
                cat("##### LOD Score Drops\n")
                par(mfrow = c(2,1))
                barplot_with_num(sort(round(mediated.lod[,1], 2)), 
                    main = "Mediated Trait Peaks", las = 2, text.gap = 0.2,
                    text.shift = 0.12)
                abline(h = trait.peak.lod)
                barplot_with_num(sort(round(mediated.lod[,2], 2)), 
                ...
                par(mar = c(2,2,2,2))
                imageWithText(scaled.expr[ind.order, trans.order], 
