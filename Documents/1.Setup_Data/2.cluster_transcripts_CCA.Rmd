...
#rankZ normalize expression
for(i in 1:length(tissue.adj.expr)){
    tissue.adj.expr[[i]] <- apply(tissue.adj.expr[[i]], 2, rankZ)
}
...
pos.cor <- TRUE
for(tx in 1:length(all.tissue.cor)){
    for(tr in 1:ncol(all.tissue.cor[[tx]])){
        test.seq <- seq(10, 2000, 10)
        all.cor <- rep(NA, length(test.seq))
        for(ts in 1:length(test.seq)){
            report.progress(ts, length(test.seq))
            pos.trans <- rownames(all.tissue.cor[[tx]])[order(abs(all.tissue.cor[[tx]][,tr]), decreasing = pos.cor)[1:test.seq[ts]]]
            ...
            all.cor[ts] <- plot.with.model(matched.mats[[tx]]$Z[,tr], tr.decomp$u[,1], plot.results = FALSE)[1]
        }
        plot(test.seq, all.cor, type = "l")       
        max.idx <- test.seq[which.max(all.cor)]
        test.trans <- rownames(all.tissue.cor[[tx]])[order(all.tissue.cor[[tx]][,tr], decreasing = TRUE)[1:max.idx]]
        ...
        test.pc <- test.decomp$u[,1,drop=FALSE]
        ...
        plot.with.model(test.pc, matched.mats[[tx]]$Z[,tr], report = "cor.test")
    }
