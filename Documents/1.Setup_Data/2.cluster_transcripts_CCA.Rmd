...
```{r imputed_measured_cor, eval = FALSE}
#if looking at imputed data, test the correlation with meaasured data
for(tx in 1:length(tissue.names)){
    measured.expr <- adjust(get(data.set.names[[tx]])$data$norm, get(data.set.names[[tx]])$covar.matrix)
    matched.expr <- get.xz(matched.mats[[tx]]$X, measured.expr)
    common.expr <- Reduce("intersect", lapply(matched.expr, colnames))
    expr.cor <- sapply(common.expr, function(x) cor(matched.expr[[1]][,x], matched.expr[[2]][,x]))
    hist(expr.cor, breaks = 100)

    plot.with.model(expr.cor, var.decomp[[tx]][common.expr,"local_VE"])
    plot.with.model(expr.cor, var.decomp[[tx]][common.expr,"distal_VE"])
    #plot.with.model(expr.cor, var.decomp[[tx]][common.expr,"covar_VE"])

    cor.col <- colors.from.values(expr.cor, use.pheatmap.colors = TRUE)
    plot(var.decomp[[tx]][common.expr,"local_VE"], var.decomp[[tx]][common.expr,"distal_VE"],
        col = cor.col, pch = 16)
    
    par(mfrow = c(1,2))
    distal.col <- colors.from.values(var.decomp[[tx]][common.expr,"distal_VE"], use.pheatmap.colors = TRUE)
    plot(expr.cor, var.decomp[[tx]][common.expr,"local_VE"], xlab = "correlation", 
        ylab = "local_VE", col = distal.col, pch = 16, cex = 0.5, main = "colored by distal VE",
        ylim = c(0,0.9))

    local.col <- colors.from.values(var.decomp[[tx]][common.expr,"local_VE"], use.pheatmap.colors = TRUE)
    plot(expr.cor, var.decomp[[tx]][common.expr,"distal_VE"], xlab = "correlation",
        ylab = "distal_VE", col = local.col, pch = 16, cex = 0.5, main = "colored by local VE",
        ylim = c(0,0.9))

    #from these we see that we can predict transcription well when
    #the variance explained by local genotype is high.
    #It also helps to have high variance explained by distal genetics (for the full model),
    #but not as much 

    gene.name = "Lep"
    gene.id <- gene.table[which(gene.table[,"symbol"] == gene.name),1]
    plot.with.model(matched.expr[[1]][,gene.id], matched.expr[[2]][,gene.id])

    
    measured.scan <- scan1(genoprobs, matched.expr[[2]][,gene.id], kinship = K)
    plot(measured.scan, map = map)

    imputed.scan <- scan1(genoprobs, matched.expr[[1]][,gene.id], kinship = K)
    plot(imputed.scan, map = map)
}
...
trait.loading.mat <- Reduce("cbind", all.loadings)
