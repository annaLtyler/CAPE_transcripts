...
for(tx in 1:length(all.tissue.cor)){
    for(tr in 1:ncol(all.tissue.cor[[tx]])){
        ...
        for(ts in 1:length(test.seq)){
            ...
            all.cor[ts,1] <- cor(matched.mats[[tx]]$Z[,tr], tr.decomp$u[,1])
            all.cor[ts,2] <- cor(matched.mats[[tx]]$Z[,tr], tr.decomp$u[,2])
        }
        ...
        par(mfrow = c(1,2))
        plot.with.model(test.pc1, matched.mats[[tx]]$Z[,tr], report = "cor.test",
            xlab = "Imputed Transcript PC1", ylab = colnames(cca.pheno)[tr])
        plot.with.model(test.pc2, matched.mats[[tx]]$Z[,tr], report = "cor.test",
